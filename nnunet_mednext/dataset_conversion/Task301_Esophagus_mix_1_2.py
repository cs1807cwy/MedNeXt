...
def make_out_dirs(task_id: int, task_name="Esophagus_mix_1_2"):
...
def create_esophagus_mix_1_2_split(labelsTr_folder: str, seed: int = 0) -> List[dict[str, List]]:
...
def copy_files(src_data_folder: Path, train_dir: Path, labels_train_dir: Path, test_dir: Path, labels_test_dir: Path):
    """Copy files from the Esophagus_mix_1_2 dataset to the nnUNet dataset folder. Returns the number of training cases."""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--input_folder",
        type=str,
        help="The downloaded Esophagus_mix_1_2 dataset dir. Should contain extracted 'pre-therapy' and 'post-therapy' folders.",
    )
    parser.add_argument(
        "-d", "--task_id", required=False, type=int, default=301, help="nnU-Net Dataset ID, default: 301"
    )
